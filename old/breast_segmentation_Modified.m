function [mask,pectoral,nipple,CannyG] = breast_segmentation_Modified(image,airskin_iter,hough_det_sens,fig,im_crop,im_direction)
...
[mask,lineprm,nipple,CannyG] = segmentPectoral_modified(im,BS_mask,hough_det_sens);
...
if fig > 0
    figure(1);
    subplot(1,2,1); imagesc(image); colormap('gray'); axis image; title('image');
    if numel(lineprm) > 0
        hold on; plot(nipple(2),nipple(1),'rs'); plot([y1,y2],[x1,x2]);
        plot([y1,y2],[x1,x2],'rs'); hold off;
    end
    subplot(1,2,2); imagesc(mask); colormap('gray'); axis image; title('mask');
    if numel(lineprm) > 0
