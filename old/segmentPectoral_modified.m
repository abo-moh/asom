function [mask,lineprm,nipple,G] = segmentPectoral_modified(im,BS_mask,hough_detsens)
...
G=edge(double(resize_im),'canny').* resize_BS_mask;
%G=double(G)*820;

% Hough transform
%[accum, axis_rho, axis_theta, lineprm] = Hough_Grd(G, 8, hough_detsens);
 [H, THETA, RHO] = HOUGH(G);
    figure(15); imagesc(THETA*(180/pi), RHO, H); axis xy;
    ...
    [indy,indx]=ind2sub(size(H),indi);
    
    l=[cos(THETA(indx)*(pi/180)) sin(THETA(indx)*(pi/180)) -RHO(indy)];
    ...
    lineprm=[RHO(indy) THETA(indx)*(pi/180)];
